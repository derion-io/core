...
import "./PositionerUP.sol";

contract HelperUP is Constants, IHelper, ERC1155Holder, NotToken {
    ...
    function aggregateAndOpen (
        ...
        // pool.swap
        {
            ...
            Result memory result = IPositioner(params.pool).transition(
                Param(
    ...
    function swapAndOpen (
        ...
        Result memory result = IPositioner(params.deriPool).transition(
            Param(
                address(this),
                payload
            ),
    ...
    function swap(SwapParams memory params) public payable returns (uint256 amountOut) {
        ...
        {
            Result memory result = IPositioner(params.poolIn).transition(
                Param(
                    address(this),
                    payload
                ),
    ...
    function _swapMultiPool(SwapParams memory params, address TOKEN_R) internal returns (uint256 amountOut) {
        ...
        {
            Result memory result = IPositioner(params.poolOut).transition(
                Param(
                    address(this),
                    payload
                ),
