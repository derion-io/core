...
contract PositionerUP is IPositioner, Constants, Fetcher {
    ...
    function initialize(Config memory config, State memory state, Payment memory payment) external {
        ...
        require(s_pos[SIDE_A][payment.recipient][0].share == 0, "SHARE_A");
        require(s_pos[SIDE_B][payment.recipient][0].share == 0, "SHARE_B");
        require(s_pos[SIDE_C][payment.recipient][0].share == 0, "SHARE_C");

        s_pos[SIDE_A][payment.recipient][0] = Position(rA, rA, 0);
        s_pos[SIDE_B][payment.recipient][0] = Position(rB, rB, 0);
        s_pos[SIDE_C][payment.recipient][0] = Position(rC, rC, 0);

        s_total[SIDE_A] = rA;
        s_total[SIDE_B] = rB;
        s_total[SIDE_C] = rC;

        emit Position(payment.recipient, SIDE_A, 0, rA, price, rA);
        emit Position(payment.recipient, SIDE_B, 0, rB, price, rB);
        emit Position(payment.recipient, SIDE_C, 0, rC, price, rC);
