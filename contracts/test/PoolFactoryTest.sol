...
import "./AsymptoticPerpetualTest.sol";

contract PoolFactoryTest is IPoolFactory {
    bytes32 constant public BYTECODE_HASH = keccak256(type(AsymptoticPerpetualTest).creationCode);
    ...
    function createPool(Params memory params) external returns (address pool) {
        t_params = params;
        pool = Create2.deploy(0, _salt(params), type(AsymptoticPerpetualTest).creationCode);
        delete t_params;
    }
