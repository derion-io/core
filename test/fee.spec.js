...
const dailyFundingRate = 0.003 // 0.3%
const HALF_LIFE = dailyFundingRate == 0 ? 0 : Math.round(86400 / Math.log2(1 / (1 - dailyFundingRate)))
...
describe(`Funding rate fee`, function () {
  async function deployDDLv2() {
    ...
    async function swapAndWait(period, amount, side) {
      ...
      expect(Number(weiToNumber(protocolFee)) / Number(weiToNumber((amount))))
