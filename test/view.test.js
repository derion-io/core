...
describe("View", function () {
  const fixture = loadFixtureFromParams(
    ...
    ],
    {
      ...
      callback: async function ({ derivablePools, uniswapPair }) {
        ...
        await pool.swap(SIDE_R, SIDE_C, numberToWei(10));

        const FlashloanAttack = await ethers.getContractFactory(
          "FlashloanAttack"
        );
        const flashloan = await FlashloanAttack.deploy(
          uniswapPair.address,
  ...
  describe("compute", function () {
    async function testCompute(side, price, closeTo = false) {
      ...
      const tokenBefore = await derivable1155.balanceOf(
        owner.address,
        packId(side, pool.contract.address)
      );
      await pool.swap(SIDE_R, side, numberToWei(1));

      const tokenAfter = await derivable1155.balanceOf(
        owner.address,
        packId(side, pool.contract.address)
      );
      ...
      const { rA, sA, rB, sB, rC, sC } = await pool.contract.compute(
      ...
      const amountOut = await pool.swap(side, SIDE_R, amountIn, {
        static: true,
      });
      if (closeTo)
        expect(Number(weiToNumber(amountIn.mul(r).div(s)))).closeTo(
          Number(weiToNumber(amountOut)),
          1e-17
        );
      else
        expect(amountIn.mul(r).div(s))
          .gte(amountOut.sub(2))
          .lte(amountOut.add(2));
    }
